...
política para esses conjuntos consiste em uma proposta mais recente do 
...
As Seções que se seguem derivam as equações do passo de predição do SEIF-SLAM a partir das equações de predição do EIF, \ref{eq:eif-info-matrix-prediction}, \ref{eq:eif-info-vector-prediction} e \ref{eq:eif-mean-prediction}. A esparsidade da matriz de informação é 
...
elementos do vetor de estados estimados.
...
obedecendo a identidade $\infov{} = \infom{}\mean{}$. Porém esse ajuste não viola o caráter constante do passo de atualização, pois a quantidade de elementos é fixa de acordo com $\lvert \bupvec{m}{+} \rvert$.}. O passo de predição do SEIF-SLAM está compilado no Algoritmo \ref{alg:seif-slam-prediction}. 
\begin{algorithm}[h]
  \setstretch{1.4}
