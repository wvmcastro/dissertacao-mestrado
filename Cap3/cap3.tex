...
Neste capítulo, serão descritas as alterações necessárias no EKF clássico para que ele possa ser aplicado na resolução do problema SLAM, estimando a pose do robô e a posição das \textit{landmarks}, o que é conhecido como EKF-SLAM e proposto por \citeonline{smith1986representation}. Além disso, também serão abordadas técnicas decorrentes do EKF-SLAM, como EIF-SLAM (Filtro de Informação Estendido) e SEIF-SLAM (Filtro de Informação Estendido Esparso) criado por \citeonline{thrun2004simultaneous}, sendo esta última a técnica de estimação utilizada neste trabalho.
...
robô quanto o mapa são estimados, o vetor de estados é composto por ambos como apresentado na Equação \ref{eq:slam-state-vector}.
\begin{equation}
...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
Em termos de implementação, isso significa que apenas as componentes da pose e seus respectivos endereços na memória são modificados no vetor de estados. Dessa forma, o passo de predição do 
...
A complexidade dessa operação é da ordem de $\bigO{n}$ (vide nota\footnote{Como discutido no Apêndice \ref{app:matrix-product-analysis}, apesar da multiplicação de matrizes possuir complexidade assintótica cúbica no pior caso, como a matriz $\bsubvec{G}{t}^\mathrm{r}$ possui dimensão $3\times 3$ e a matriz $\cov{t-1}^\mathrm{rm}$ possui dimensão $3 \times (n-3)$ (onde $n$ é o tamanho do vetor de estados) há apenas uma dimensão variável (a quantidade de colunas da matriz $\cov{t-1}^\mathrm{rm}$) e portanto a complexidade se torna $\bigO{n}$.}) por conta do termo $\bsubvec{G}{t}^\mathrm{r} \cov{t-1}^\mathrm{rm}$, enquanto no caso geral do EKF em que o jacobiano $\bsubvec{G}{t}$ é denso, essa complexidade é $\bigO{n^{2.376}}$, que é a complexidade da multiplicação de matrizes $n \times n$ \cite{coppersmith1987matrix}. A Figura 
...
correlacionadas, mesmo que muitas dessas correlações sejam pouco significativas. 
...
em que, \begin{itemize}
  \item $\predinfom{t}$ é a matriz de informação predita
  \item $\predinfov{t}$ é o vetor de informação predito
\end{itemize}
e a distribuição de probabilidade na Equação \ref{eq:moments-gaussian-probabilistic-distribution} pode ser reescrita da seguinte forma:
\begin{equation}
  \pr{\bvec{x}} = \eta \exp\parentheses{-\frac{1}{2} 
  \bvecT{x} \infom{} \bvec{x} + \bvecT{x}\infov{}}
  \label{eq:canonical-gaussian-probabilistic-distribution}
\end{equation}
como demonstrado em \cite[p.~74 e 75]{thrun2005probabilistic}.
...
seja, agregam pouca informação ao sistema, como mostrado na imagem da direita na Figura \ref{fig:cov-and-info-matrix-comparison}. Isso se deve à estrutura do problema 
...
\begin{figure}
  \includegraphics[width=.95\textwidth]{figs/seif-motivation.png}
  \caption[Comparação entre a matriz de covariância e a matriz de informação se um sistema com 50 \textit{landmarks}]{Comparação entre a matriz de covariância e a matriz de informação de um sistema com 50 \textit{landmarks}. À esquerda, mapa de \textit{landmarks} do sistema; ao centro, a matriz de covariância do EKF-SLAM e, à direita, a matriz de informação. Retirado de \citeonline{thrun2004simultaneousbook}.} 
  \label{fig:cov-and-info-matrix-comparison}
\end{figure}
...
em que vizinhança é definido como o conjunto de \textit{landmarks} presentes em 
...
Embora o conceito dos conjuntos de \textit{landmarks} ativas e passivas 
seja introduzido por \citeonline{thrun2004simultaneous}, não é dada uma 
descrição detalhada de política para construir e manter esses conjuntos 
ao longo do tempo. O único trabalho encontrado que descreve uma 
política para esses conjuntos consiste em uma proposta do 
SEIF denotada Filtro de Informação Estendido Exato (ESEIF) \cite{walter2007exactly}.

No entanto, neste trabalho é implementada uma política diferente de \citeonline{walter2007exactly}; aqui o conjunto $\bsubvec{m}{t}^+$ contém as $k$ últimas \textit{landmarks} 
observadas até o instante $t$, em que $k$ é a cardinalidade do conjunto. As 
\textit{landmarks} vão entrando e saindo desse conjunto conforme o robô 
navega no ambiente e novas \textit{landmarks} vão sendo observadas enquanto outras deixam de sê-lo, conforme exemplo na Tabela \ref{tab:active-landmarks-buffer}.

\begin{table}[]
\caption{Exemplo da política do conjunto de \textit{landmarks} ativas 
implementada, com cardinalidade $k = 3$.}
\label{tab:active-landmarks-buffer}
\centering
\begin{tabular}{cccc}
\hline
Tempo & Landmarks observadas & $\bupvec{m}{+}$ & $\bupvec{m}{-}$ \\ \hline
$t_0$ & $\{\}$ & $\{\}$ &  $\{\}$\\
$t_1$ & $\{1,2,3\}$ & $\{1,2,3\}$ & $\{\}$ \\
$t_2$ &  $\{2,3,4\}$ & $\{2,3,4\}$ &  $\{1\}$\\
$t_3$ &  $\{3,4\}$ & $\{2,3,4\}$ &  $\{1\}$\\
...
Note que o conjunto de \textit{landmarks} ativas $\bupvec{m}{+}$ em um dado instante $t$ não é 
necessariamente composto pelas \textit{landmarks} observadas neste instante, como 
evidenciado na linha $t_3$ da Tabela \ref{tab:active-landmarks-buffer}. Quando um subconjunto de \textit{landmarks} 
presentes em $\bupvec{m}{+}$ foram inseridas dentro de um mesmo passo 
de execução e precisa-se escolher alguma para ser substituída, 
escolhe-se a de menor índice como mostrado na linha $t_2$ da Tabela \ref{tab:active-landmarks-buffer}. 
...
As Seções que se seguem derivam as equações do passo de predição do SEIF-SLAM a partir das equações \ref{eq:eif-info-matrix-prediction} a \ref{eq:eif-info-vector-prediction} de predição do EIF. A esparsidade da matriz de informação é 
...
\end{equation}
em que $\bvec{M}$ é uma a matriz de projeção de um subconjunto dos 
elementos do vetor de estado estimado.

\subsubsection{Predição da Matriz de Informação}
Primeiro, vamos reescrever a Equação \ref{eq:eif-info-matrix-prediction} em 
termos de $\bsubvec{\Phi}{t}$ e, $\bsubvec{R}{t}^r$:
\begin{equation}
...
obedecendo a identidade $\infov{} = \infom{}\mean{}$. Porém, esse ajuste não viola o caráter constante do passo de atualização, pois a quantidade de elementos é fixa de acordo com $\lvert \bupvec{m}{+} \rvert$.}. O passo de predição do SEIF-SLAM é apresentado no Algoritmo \ref{alg:seif-slam-prediction}. 
\begin{algorithm}[h]
  \setstretch{1.4}
...
fundamental para as linearizações dos modelos de movimento e medida, e para computar o vetor de informação na etapa de esparsificação. Na Equação 
\ref{eq:eif-mean-recuperation} do Filtro de Informação Estendido, essa 
computação usa a inversão da matriz de informação 
e, mesmo numa matriz esparsa como a do SEIF, o tempo requerido para essa operação não é constante.
...
Como pode ser notado na linha 3 do Algoritmo \ref{alg:seif-slam-mean-recovery}, dentre todas as 
