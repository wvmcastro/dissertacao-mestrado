...
\begin{figure}[h]
  ...
  \caption{O vetor média e a matriz de covariância são completamente atualizados durante a observação de uma \textit{landmark}. Retirado de \cite[p.~8]{jsola}.}
...
dado que $\mathrm{K}$ e o cardinalidade do conjunto $\bvec{m}^+$ são ambos 
constantes.
...
Para realizar a esparsificação da matriz de informação do SEIF-SLAM, vamos retomar os conjuntos de \textit{landmarks} $\bsubvec{m}{t}^+$ e $\bsubvec{m}{t}^-$, ativas e inativas, respectivamente e já definidos na Seção \ref{sec:seif-active-passive-landmarks}. Também 
...
\begin{itemize}
  ...
  \textit{landmarks} ativas $\bsubvec{m}{t}^+$ que serão tornadas inativas, 
...
para a medida lida. E caso essa distância seja menor que um limiar $\dataTh$, essa medida é associada à 
...
\begin{algorithmic}[1]
  ...
  \While{$m < \mathrm{M}$}
    ...
    \If{$\pi_m < d$ \textbf{and} $\pi_m < \dataTh$}
      \State $c \gets \{m\}$
    \EndIf
    \State $m \gets m + 1$
  \EndWhile
