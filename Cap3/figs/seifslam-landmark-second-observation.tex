...
\begin{tikzpicture}
  ...
  % % active landmarks
    ...
    \foreach \activeLandmark[count=\i] in \activeLandmarks {
      \draw [ultra thick] (\robotpos) -- (\activeLandmark) node[midway] (L_\i) {};
      % \foreach \pos[count=\j] in \activeLandmarks{
      %   \ifnum \i<\j
      %     \draw [thick] (\activeLandmark) -- (\pos);
      %   \fi
      % }
    }
  ...
  % % label
  \node (RL)at(\robotposx+.5cm, \m-0.5) {$\robotstate{t}$};
  %

  % % arcs
  \path[latex-latex, thick, bend right] (2.5,\m-0.5) edge node [pos=.65, anchor=south, sloped] {conexão} (L_2);
