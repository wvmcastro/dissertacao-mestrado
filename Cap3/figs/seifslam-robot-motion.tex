...
\begin{tikzpicture}
  ...
  \def\activeLandmarks{\landmark1, \landmark2}
  \def\passiveLandmarks{\landmark3}
    ...
    \tikzmath{
      for \i in {1,2}{
        for \j in {1,2}{
          if \i != \j then{
            {
              \filldraw[fill=magenta!70, thick] (\j, \m-\i-1) rectangle ++(1,1);
            };
          };
        };
      };
  ...
  % % active landmarks
    % links
    \foreach \activeLandmark[count=\i] in \activeLandmarks {
      \draw [thick, dashed] (\robotpos) -- (\activeLandmark) node[midway] (L_\i) {};
      \foreach \pos[count=\j] in \activeLandmarks{
        \ifnum \i<\j
          \draw [very thick, magenta] (\activeLandmark) -- (\pos) node[midway] (L_\i_\j) {};
        \fi
      }
    }
  ...
  % % arcs
  \path[latex-latex, thick, bend left] (2.5,\m-1.5) edge node [pos=0.6, anchor=north, sloped, align=center] {conexão de\\movimento} (L_1_2);
